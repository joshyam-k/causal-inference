...
$$
P\bigg(\Big|X_{(n)} - E[X_{(n)}]\Big| \ge \varepsilon\bigg) \le \frac{\sqrt{var(X_{(n)})}}{\varepsilon^2}
$$

And since we know that $var(X_{(n)})\rightarrow 0$, then by the continuous mapping theorem we know that 

$$
\frac{\sqrt{var(X_{(n)})}}{\varepsilon^2} \rightarrow \frac{\sqrt{(0)}}{\varepsilon^2} = 0
$$
