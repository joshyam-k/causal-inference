...
randomization_scheme <- function() {
  ...
  stage2_labels <- data_s1 %>% 
    filter(treat1 == "LPP+Subsidy") %>% 
    distinct(cid) %>% 
    mutate(stage2grp = sample(c(rep("LPP+Subsidy", n1), rep("LPP+Subsidy+Supply", n2)), 226)) %>% 
    group_by(stage2grp) %>% 
    mutate(intensity = sample(c("High", "Medium", "Low"), 113, replace = T))
...
stage2_labels <- data_s1 %>% 
  filter(treat1 == "LPP+Subsidy") %>% 
  distinct(cid) %>% 
  mutate(stage2grp = sample(c(rep("LPP+Subsidy", 113), rep("LPP+Subsidy+Supply", 113)), 226)) %>% 
  group_by(stage2grp) %>% 
  mutate(intensity = sample(c("High", "Medium", "Low"), 113, replace = T))

data_s2 <- data_s1 %>% 
  left_join(stage2_labels, by = "cid") %>% 
...
lottery_labels <- data_s2 %>% 
  filter(treat2 %in% c("High", "Medium", "Low")) %>% 
  rowwise() %>% 
  mutate(lottery = case_when(
    treat2 == "Low" ~ sample(c("Winner", "Loser"), 1, prob = c(0.4, 0.6)),
    treat2 == "Medium" ~ sample(c("Winner", "Loser"), 1, prob = c(0.5, 0.5)),
    treat2 == "High" ~ sample(c("Winner", "Loser"), 1, prob = c(0.6, 0.4))
  )) %>% 
  ungroup() %>% 
