...
two_sided <- null_dist %>% 
  ...
  geom_vline(xintercept = -abs(true_dim), color = "red", linetype = "dashed", linewidth = 1.2) +
  geom_vline(xintercept = abs(true_dim), color = "red", linetype = "dashed", linewidth = 1.2) +
  scale_fill_manual(values = c("#a9ccaa", "#4c5c4d")) +
  theme_minimal() +
  annotate(geom = "text", x = -0.11, y = 150, label = "p-value = 0.17", color = "#d11611" )
```
...
```{r}
grid <- true_dim + seq(from=-4, to=4, length.out=200) 

Null_Dist_Matrix <- NULL
for(i in 1:1000){
  d_new <- randomization_scheme()$treat3
  New_DIM <- data.frame(rep(0, 200))
  for(v in grid){
    data_raw %>% 
      mutate(D_new = d_new) %>% 
      mutate(
    ...
    # Create new POs based on null value
    y0_h0 <- y - d*v
    y1_h0 <- y + (1-d)*v
    y_new <- d_new*y1_h0 + (1-d_new)*y0_h0
    # New DIM
    fit <- lm(y_new~d_new)
    dim_new <- fit$coefficients["d_new"]
    New_DIM <- c(New_DIM, dim_new)
  } # end of v
  Null_Dist_Matrix <- rbind(Null_Dist_Matrix, New_DIM)
} 
```
