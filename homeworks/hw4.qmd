...
must include Z or C or both and then any combination of $B, A, X, W$, can also just include C only.
...
For collider there are no backdoor paths into $X$. I suppose $\{\}$ sort of vacuously satisfies the backdoor criterion which indeed gives us

$$
P(Y \ | \ \text{do}(X = x)) = \sum_{\{\}}P(Y | X = x) = P(Y | X = x)
$$
